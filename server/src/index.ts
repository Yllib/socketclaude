...
const ADB_BRIDGE_SIDECAR_STATE = socketAgentDataPath("adb-bridge-sidecar.json");
const ADB_BRIDGE_SIDECAR_LOG = socketAgentDataPath("adb-bridge-sidecar.log");

type AdbBridgeSidecarState = {
  pid: number;
  localHost: string;
  localPort: number;
  startedAt: string;
...
const pendingPhoneAdbRequests = new Map<string, PendingPhoneAdbRequest>();
const PHONE_ADB_FILE_CHUNK_SIZE = 512 * 1024;

type PhoneAdbRequestOptions = {
  command: "devices" | "pair" | "connect" | "shell" | "adb" | "install" | "open_apk" | "logcat";
  shellCommand?: string;
  args?: string[];
  pairPort?: number;
  connectPort?: number;
...
function readAdbBridgeSidecarState(): AdbBridgeSidecarState | null {
  try {
    if (!fs.existsSync(ADB_BRIDGE_SIDECAR_STATE)) return null;
    const parsed = JSON.parse(fs.readFileSync(ADB_BRIDGE_SIDECAR_STATE, "utf8"));
    const pid = Number(parsed.pid);
    const localPort = Number(parsed.localPort);
    if (!Number.isInteger(pid) || pid <= 0 || !Number.isInteger(localPort)) {
      return null;
    }
    return {
      pid,
      localHost: typeof parsed.localHost === "string" && parsed.localHost.trim()
        ? parsed.localHost.trim()
        : "127.0.0.1",
      localPort,
      startedAt: typeof parsed.startedAt === "string" ? parsed.startedAt : "",
      logPath: typeof parsed.logPath === "string" && parsed.logPath.trim()
        ? parsed.logPath.trim()
        : ADB_BRIDGE_SIDECAR_LOG,
    };
  } catch {
    return null;
  }
...
function writeAdbBridgeSidecarState(state: AdbBridgeSidecarState): void {
  fs.writeFileSync(ADB_BRIDGE_SIDECAR_STATE, JSON.stringify(state, null, 2));
}

function clearAdbBridgeSidecarState(): void {
  try {
    if (fs.existsSync(ADB_BRIDGE_SIDECAR_STATE)) fs.unlinkSync(ADB_BRIDGE_SIDECAR_STATE);
  } catch {}
}
...
function adbBridgeSidecarStatus(requestId?: string): Record<string, unknown> {
  const state = readAdbBridgeSidecarState();
  if (!state) {
    return {
      type: "adb_bridge_sidecar_status",
      requestId,
      running: false,
      logPath: ADB_BRIDGE_SIDECAR_LOG,
    };
  }
  if (!isProcessAlive(state.pid)) {
    clearAdbBridgeSidecarState();
    return {
      type: "adb_bridge_sidecar_status",
      requestId,
      running: false,
      ...
      error: "ADB bridge sidecar is not running.",
    };
  }
  return {
    type: "adb_bridge_sidecar_status",
...
function normalizeAdbBridgeLocalPort(raw: unknown): number {
  const localPort = Number(raw || process.env.SOCKETAGENT_ADB_BRIDGE_PORT || 5038);
  if (!Number.isInteger(localPort) || localPort <= 0 || localPort > 65535) {
    throw new Error(`Invalid ADB bridge local port: ${raw}`);
  }
  return localPort;
}

function startAdbBridgeSidecar(requestId?: string, localPortInput?: unknown): Record<string, unknown> {
  const localHost = "127.0.0.1";
  const localPort = normalizeAdbBridgeLocalPort(localPortInput);
  const existing = adbBridgeSidecarStatus();
  if (existing.running === true) {
    if (existing.localPort === localPort && existing.localHost === localHost) {
      return {
        ...existing,
        requestId,
        type: "adb_bridge_sidecar_status",
        message: "ADB bridge sidecar is already running.",
      };
    }
    stopAdbBridgeSidecar();
  }

  const script = path.join(__dirname, "adb-bridge.js");
  if (!fs.existsSync(script)) {
    throw new Error("ADB bridge sidecar is not compiled on this server yet.");
  }

  const logFd = fs.openSync(ADB_BRIDGE_SIDECAR_LOG, "a");
  try {
    fs.writeSync(logFd, `\n[${new Date().toISOString()}] Starting ADB bridge sidecar on ${localHost}:${localPort}\n`);
    const child = spawn(
      process.execPath,
      [script, "--local-host", localHost, "--local-port", String(localPort)],
      {
    ...
    if (!child.pid) {
      throw new Error("Failed to start ADB bridge sidecar.");
    }
    const state: AdbBridgeSidecarState = {
      pid: child.pid,
      localHost,
      localPort,
    ...
    writeAdbBridgeSidecarState(state);
    child.unref();
    return {
      type: "adb_bridge_sidecar_status",
      requestId,
      ...
      startedAt: state.startedAt,
      logPath: state.logPath,
      message: "ADB bridge sidecar started.",
    };
  } finally {
    try {
      fs.closeSync(logFd);
...
function stopAdbBridgeSidecar(requestId?: string): Record<string, unknown> {
  const state = readAdbBridgeSidecarState();
  if (!state) {
    return {
      type: "adb_bridge_sidecar_status",
      ...
      running: false,
      logPath: ADB_BRIDGE_SIDECAR_LOG,
      message: "ADB bridge sidecar is not running.",
    };
  }
  if (isProcessAlive(state.pid)) {
    try {
      ...
      return {
        ...
        startedAt: state.startedAt,
        logPath: state.logPath,
        error: `Failed to stop ADB bridge sidecar: ${e?.message || String(e)}`,
      };
    }
  }
  clearAdbBridgeSidecarState();
  return {
    ...
    localPort: state.localPort,
    logPath: state.logPath,
    message: "ADB bridge sidecar stopped.",
  };
}

function normalizePhoneAdbArgs(raw: unknown): string[] {
...
function normalizePhoneAdbPort(raw: unknown, label: string): number {
...
function sendPhoneAdbPayload(payload: Record<string, unknown>): number {
  ...
  if (relayConnectionHandler) {
    relayConnectionHandler.sendRaw(message);
    sent++;
  }
  return sent;
}

function cancelPhoneAdbRequest(requestId: string, reason = "Phone ADB request cancelled."): void {
  const pending = pendingPhoneAdbRequests.get(requestId);
  if (pending) {
    pendingPhoneAdbRequests.delete(requestId);
    clearTimeout(pending.timer);
    pending.reject(new Error(reason));
  }
  sendPhoneAdbPayload({ type: "phone_adb_cancel", requestId });
}

async function streamPhoneAdbFile(requestId: string, filePath: string): Promise<void> {
  let chunkIndex = 0;
  await new Promise<void>((resolve, reject) => {
    const stream = fs.createReadStream(filePath, { highWaterMark: PHONE_ADB_FILE_CHUNK_SIZE });
...
async function sendPhoneAdbRequest(
  options: PhoneAdbRequestOptions,
  requestId = `phone_adb_${Date.now()}_${Math.random().toString(36).slice(2)}`
): Promise<Record<string, unknown>> {
  const args = normalizePhoneAdbArgs(options.args || []);
  const timeoutSeconds = Math.max(1, Math.min(Number(options.timeoutSeconds || 60), 900));
  const maxBytes = Math.max(1024, Math.min(Number(options.maxBytes || 1024 * 1024), 64 * 1024 * 1024));

  if (options.command === "shell" && !String(options.shellCommand || "").trim()) {
    throw new Error("Missing shell command.");
  }
  if (options.command === "pair") {
    normalizePhoneAdbPort(options.pairPort, "pairing");
    if (!String(options.code || "").trim()) {
      throw new Error("Missing pairing code.");
    }
  }
  if (options.command === "connect") {
    normalizePhoneAdbPort(options.connectPort, "connect");
  }
  if (options.command === "adb" && args.length === 0) {
    throw new Error("Missing adb arguments.");
  }
  let fileSize = 0;
  let fileName = "";
  if (options.command === "install" || options.command === "open_apk") {
    if (!options.filePath) throw new Error("Missing APK path.");
    const stat = fs.statSync(options.filePath);
    if (!stat.isFile()) throw new Error("APK path is not a file.");
    fileSize = stat.size;
  ...
  const resultPromise = new Promise<Record<string, unknown>>((resolve, reject) => {
    const timer = setTimeout(() => {
      pendingPhoneAdbRequests.delete(requestId);
      reject(new Error("Timed out waiting for phone ADB result."));
    }, (timeoutSeconds + 30) * 1000);
    pendingPhoneAdbRequests.set(requestId, {
      resolve,
      reject,
      timer,
  ...
  const sent = sendPhoneAdbPayload({
    type: "phone_adb_request",
    requestId,
    command: options.command,
    shellCommand: options.command === "shell" ? String(options.shellCommand || "") : undefined,
    args,
    pairPort: options.command === "pair" ? options.pairPort : undefined,
    connectPort: options.command === "connect" ? options.connectPort : undefined,
    code: options.command === "pair" ? String(options.code || "") : undefined,
    timeoutSeconds,
    maxBytes,
  ...
  if (sent === 0) {
    ...
    throw new Error("No connected SocketAgent app is available for phone ADB.");
  }

  if ((options.command === "install" || options.command === "open_apk") && options.filePath) {
    try {
      await streamPhoneAdbFile(requestId, options.filePath);
    } catch (e: any) {
      cancelPhoneAdbRequest(requestId, e?.message || String(e));
      throw e;
    }
  }
...
function normalizeAdbHost(raw: unknown): string {
  const host = String(raw || "").trim();
  if (!host || host.length > 255 || /[\s/\\'"`;|&<>$]/.test(host)) {
    throw new Error("Invalid ADB host.");
  }
  return host;
}

function runAdbDirectCommand(
  ...
  const adbCommand = command === "connect" ? "connect" : command === "pair" ? "pair" : "";
  if (!adbCommand) throw new Error("Unsupported ADB command.");
  const host = normalizeAdbHost(hostInput);
  const port = normalizeAdbBridgeLocalPort(portInput);
  const endpoint = `${host}:${port}`;
  const args = adbCommand === "pair"
    ? ["pair", endpoint, String(codeInput || "").trim()]
    : ["connect", endpoint];
  if (adbCommand === "pair" && !args[2]) {
...
function createConnectionHandler(
  ...
  async function handleMessage(msg: ClientMessage): Promise<void> {
    ...
    switch (msg.type) {
      ...
      case "adb_bridge_sidecar_start": {
        const requestId = typeof (msg as any).requestId === "string" ? (msg as any).requestId : undefined;
        try {
          sendJson(startAdbBridgeSidecar(requestId, (msg as any).localPort));
        } catch (e: any) {
          sendJson({
            type: "adb_bridge_sidecar_status",
            requestId,
      ...
      case "adb_bridge_sidecar_stop": {
        const requestId = typeof (msg as any).requestId === "string" ? (msg as any).requestId : undefined;
        sendJson(stopAdbBridgeSidecar(requestId));
        break;
      }

      case "adb_bridge_sidecar_status": {
        const requestId = typeof (msg as any).requestId === "string" ? (msg as any).requestId : undefined;
        sendJson(adbBridgeSidecarStatus(requestId));
        break;
      }
      ...
      case "phone_adb_result": {
        const requestId = typeof (msg as any).requestId === "string" ? (msg as any).requestId : "";
        const pending = pendingPhoneAdbRequests.get(requestId);
        if (pending) {
          pendingPhoneAdbRequests.delete(requestId);
          clearTimeout(pending.timer);
          pending.resolve(((msg as any).result || {}) as Record<string, unknown>);
        }
        break;
      }

      case "phone_adb_stream_chunk": {
        const requestId = typeof (msg as any).requestId === "string" ? (msg as any).requestId : "";
        const pending = pendingPhoneAdbRequests.get(requestId);
        if (pending?.onChunk) {
          pending.onChunk({
            stream: typeof (msg as any).stream === "string" ? (msg as any).stream : "stdout",
            data: typeof (msg as any).data === "string" ? (msg as any).data : "",
          });
        }
        break;
      }
...
const httpServer = http.createServer((req, res) => {
  ...
  // POST /phone-adb — ask the connected phone app to run local ADB.
  if (req.method === "POST" && req.url?.startsWith("/phone-adb")) {
    const url = new URL(req.url, `http://localhost:${PORT}`);
    ...
    req.on("end", async () => {
      try {
        ...
        const rawCommand = String(parsed.command || "devices");
        const command: PhoneAdbRequestOptions["command"] =
          rawCommand === "shell" ? "shell"
            : rawCommand === "pair" ? "pair"
              : rawCommand === "connect" ? "connect"
            : rawCommand === "adb" || rawCommand === "command" ? "adb"
              : rawCommand === "install" ? "install"
                : rawCommand === "open_apk" || rawCommand === "open-apk" || rawCommand === "stage_apk" || rawCommand === "stage-apk" ? "open_apk"
                  : rawCommand === "logcat" ? "logcat"
                    : "devices";
        const args = normalizePhoneAdbArgs(parsed.args);
        const timeoutSeconds = Number(parsed.timeoutSeconds || (command === "install" || command === "open_apk" ? 600 : 60));
        const maxBytes = Number(parsed.maxBytes || 1024 * 1024);
        const rawFilePath =
          typeof parsed.filePath === "string" ? parsed.filePath
            : typeof parsed.apkPath === "string" ? parsed.apkPath
              : "";

        if (command === "logcat" && parsed.stream === true) {
          const requestId = `phone_adb_${Date.now()}_${Math.random().toString(36).slice(2)}`;
          let finished = false;
          res.writeHead(200, { "Content-Type": "text/plain; charset=utf-8" });
          res.on("close", () => {
            if (!finished) cancelPhoneAdbRequest(requestId, "Phone ADB logcat client disconnected.");
          });
          try {
            const result = await sendPhoneAdbRequest({
              command,
              args,
              timeoutSeconds,
              maxBytes,
              onChunk: (chunk) => {
                if (!res.writableEnded) res.write(chunk.data);
              },
            }, requestId);
            finished = true;
            const message = typeof result.message === "string" ? result.message : "";
            if (message && !res.writableEnded) res.write(`\n${message}\n`);
        ...
        const result = await sendPhoneAdbRequest({
          command,
          shellCommand: typeof parsed.shellCommand === "string" ? parsed.shellCommand : undefined,
          args,
          pairPort: command === "pair" ? normalizePhoneAdbPort(parsed.pairPort, "pairing") : undefined,
          connectPort: command === "connect" ? normalizePhoneAdbPort(parsed.connectPort, "connect") : undefined,
          code: command === "pair" ? String(parsed.code || "") : undefined,
          timeoutSeconds,
          maxBytes,
          filePath: (command === "install" || command === "open_apk") && rawFilePath ? path.resolve(rawFilePath) : undefined,
        });
        res.writeHead(200, { "Content-Type": "application/json" });
